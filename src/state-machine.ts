...
export interface LoopConfig {
  ...
  gateRegistry: ReadonlyMap<string, { command: string }>
}

export type LoopInput =
  | { type: 'contract-armed'; actor: Actor }
  | { type: 'risky-tool-before'; tool: string }
  | { type: 'model-claim'; actor: Actor; claim: ClaimSpec }
  | { type: 'tool-result'; tool: string }
  | { type: 'gate-result'; gateId: string; result: GateResult }
  | { type: 'model-hypotheses'; actor: Actor; hypotheses: HypothesisSpec[] }
  | { type: 'experiment-result'; experimentCommand: string; result: GateResult }
  | { type: 'human-escalation'; decision: 'resume' | 'disarm' }
  | { type: 'disarm' }
...
export class FalsificationLoop {
  ...
  private noveltyRejects = 0
  private activeClaim: ClaimSpec | null = null
  private lastExperimentCommands: string[] = []
  ...
  private applyEscalation(decision: 'resume' | 'disarm'): LedgerEntry {
    if (decision === 'disarm') {
      this.state = 'disarmed'
...
export function isValidClaim(c: ClaimSpec): boolean {
  return (
