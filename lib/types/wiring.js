...
import { FalsificationLoop, makeClaimId } from "./state-machine.js";
import { Ledger } from "./ledger.js";
import { FALSIFICATION_LEDGER_EVENT } from "./types-events.js";
import { armScope, foldPlanActive } from "./plan-arming.js";
const PLUGIN_SOURCE = { kind: 'plugin', plugin: 'popper' };
...
export function runGate(command, timeoutMs, cap) {
...
function durableAppend(session, entry) {
    ...
    try {
        appendRecord(FALSIFICATION_LEDGER_EVENT, entry);
        return true;
    }
    catch {
        return false;
 ...
 */
export function installWiring(ctx, config) {
    ...
    function controller(agent) {
        ...
        if (!c) {
            ...
            if (armScope(config.mode, planComposed(), planActiveNow(agent.session))) {
                for (const entry of c.loop.handle({ type: 'contract-armed', actor: 'human' }))
    ...
    function lastClaim(c) {
        for (const e of [...c.ledger.all].reverse())
            if (e.kind === 'claim' && e.claim)
                return e.claim;
        return undefined;
    }
    /** 前沿耗尽 → 请求人工裁决；无应答者/失败时记录 escalation 需求并锁存（只记录一次）。 */
    async function maybeEscalate(c, agent) {
        if (c.loop.currentState !== 'frontier-exhausted' || c.escalationPending)
            return;
        ...
        try {
            ...
            record(c, { actor: 'plugin', kind: 'protocol', verdict: 'unknown', message: 'escalation required: ask failed (' + String(err.message ?? err).slice(0, 120) + '); manual resume/disarm needed' });
        }
    }
    async function routeExecution(c, exec, inject) {
        ...
        if (config.mode === 'strict' && planComposed() && !planActiveNow(c.session))
            return [];
        // 每会话一次：播报当前模式与武装状态，让安装即可见（observe 也不静默）。
        if (!announced.has(c.session.id)) {
            announced.add(c.session.id);
            if (config.mode === 'strict')
                inject(notice(s === 'disarmed' ? disarmedBanner : armedBanner([...config.gateRegistry.keys()]), 'popper status'));
            else
                inject(notice(observeBanner, 'popper status'));
        }
        if (s === 'frontier-exhausted' || s === 'escalated')
            await maybeEscalate(c, exec.agent);
        ...
        if (config.mode === 'observe' || s === 'disarmed')
            return [];
