...
export interface FalsificationCallArgs {
  ...
  hypotheses?: { text: string; predictedObservable: string; experimentCommand: string }[]
  experimentCommand?: string
}

/** 模型可见工具定义；execute 只回显校验后的参数（无副作用），路由靠工具名在 post-execute 完成。 */
export function falsificationTool(name: string) {
  return defineTool({
    name,
    ...
    presentCall(args) {
      const a = args as unknown as FalsificationCallArgs
